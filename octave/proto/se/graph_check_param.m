function graph_check_param(graph, param_id, expected_type)
  ...
  if exist("expected_type")
    graph.params{param_id}
    param_type = graph.params{param_id}.type;
    err_msg = ["param.type [", param_type, "] != [", expected_type, "] !"];
    assert(graph.params{param_id}.type == expected_type, err_msg);
  endif
endfunction
