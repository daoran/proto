...
% Settings
calib_data_dir = "/data/intel_d435i/calib_data/aprilgrid0/cam0/data";
image_width = 752;
image_height = 480;
...
time = [];
ts0 = 0;
files = list_dir(calib_data_dir);

for i = 1:length(files)
  fname = files(i).name;
  data_path = join_paths(calib_data_dir, fname);
  [retval, aprilgrid] = load_aprilgrid(data_path);

  keypoints = [keypoints, aprilgrid.keypoints];
  fiducial_pos = [fiducial_pos, aprilgrid.r_CF];
  fiducial_rpy = [fiducial_rpy, rad2deg(quat2euler(aprilgrid.q_CF))];
  if ts0 == 0
    ts0 = aprilgrid.ts;
    time = [0.0];
  else
