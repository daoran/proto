...
void MSCKF::H(const FeatureTrack &track,
  ...
  for (int i = 0; i < M; i++) {
    ...
    const vec3_t p_C_f = C_CiG * (p_G_f - p_G_Ci);
...
int MSCKF::initialize(const long ts,
  ...
  this->imu_state.g_G = vec3_t(0.0, 0.0, -g_imu.norm());
...
void MSCKF::augmentState() {
  ...
  const vec4_t imu_q_IG = this->imu_state.q_IG;
  const vec3_t imu_p_G = this->imu_state.p_G;
  const vec4_t cam_q_CG = quatlcomp(this->ext_q_CI) * imu_q_IG;
  const vec3_t cam_p_G = imu_p_G + C(imu_q_IG).transpose() * this->ext_p_IC;
...
int MSCKF::chiSquaredTest(const matx_t &H, const vecx_t &r, const int dof) {
...
int MSCKF::residualizeTrack(const FeatureTrack &track,
  ...
  // Calculate residuals
  vecx_t r_j = zeros(2 * track_cam_states.size(), 1);
  for (size_t i = 0; i < track_cam_states.size(); i++) {
    // Transform feature from global frame to i-th camera frame
    const mat3_t C_CG = C(track_cam_states[i].q_CG);
    const vec3_t p_C_f = C_CG * (p_G_f - track_cam_states[i].p_G);
    const double u = p_C_f(0) / p_C_f(2);
    const double v = p_C_f(1) / p_C_f(2);
    const vec2_t z_hat{u, v};
...
int MSCKF::calcResiduals(const FeatureTracks &tracks, matx_t &T_H, vecx_t &r_n) {
  ...
  if (H_o.rows() > H_o.cols() && this->enable_qr_trick) {
    ...
    if (this->qr_mode == "sparse") {
      ...
      matx_t Q1 = Q.leftCols(imu_state.size + N * CameraState::size);
...
void MSCKF::correctIMUState(const vecx_t &dx) {
  const vecx_t dx_imu = dx.block(0, 0, imu_state.size, 1);
  this->imu_state.correct(dx_imu);
}

void MSCKF::correctCameraStates(const vecx_t &dx) {
  for (int i = 0; i < this->N(); i++) {
    const int rs = imu_state.size + CameraState::size * i;
    const vecx_t dx_cam{dx.block(rs, 0, CameraState::size, 1)};
