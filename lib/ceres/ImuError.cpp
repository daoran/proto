...
MatX ImuError::formF(const int k,
  ...
  const Mat3 dC_i = quat2rot(dq_i);
  const Mat3 dC_j = quat2rot(dq_j);
...
void ImuError::propagate() {
  ...
  for (int k = 0; k < (imu_buffer_.getNumMeasurements() - 1); k++) {
    ...
    const Vec3 a_k = imu_buffer_.getAcc(k);
    const Vec3 a_kp1 = imu_buffer_.getAcc(k + 1);
    const Vec3 acc_i = quat_rot(dq_i, a_k - ba_i);
    const Vec3 acc_j = quat_rot(quat_mul(dq_i, dq_perturb), a_kp1 - ba_i);
    const Vec3 a = 0.5 * (acc_i + acc_j);

    // Propagate IMU state using mid-point method
    const Vec4 dq_j = quat_mul(dq_i, dq_perturb);
...
Vec4 ImuError::getRelativeRotation() const { return dq_; }
...
bool ImuError::eval(double const *const *params,
  ...
  const Vec4 q_i = tf_quat_vec(T_i);
  ...
  const Vec4 q_j = tf_quat_vec(T_j);
  ...
  const Vec4 dq_theta{1.0, dhtheta.x(), dhtheta.y(), dhtheta.z()};

  // -- Correct the relative position, velocity and rotation
  const Vec3 dr = dr_ + dr_dba * dba + dr_dbg * dbg;
  const Vec3 dv = dv_ + dv_dba * dba + dv_dbg * dbg;
  const Vec4 dq = quat_normalize(quat_mul(dq_, dq_theta));
  ...
  if (jacs[1]) {
    const Quat dq_ji{C_j.transpose() * C_i * quat2rot(dq_)};
  ...
  if (jacs[2]) {
    const Vec4 error_rot = quat_mul(dq_inv, quat_mul(qi_inv, q_j));
