...
struct EurocImu {
  ...
  EurocImu(const std::string &data_dir_) : data_dir{data_dir_} {
   ...
   */
  friend std::ostream &operator<<(std::ostream &os, const EurocImu &data) {
    // clang-format off
...
struct EurocCamera {
   ...
   */
  friend std::ostream &operator<<(std::ostream &os, const EurocCamera &data) {
    // clang-format off
    os << "sensor_type: " << data.sensor_type << std::endl;
    os << "comment: " << data.comment << std::endl;
    os << "T_BS:\n" << data.T_BS << std::endl;
...
struct EurocGroundTruth {
  ...
  EurocGroundTruth(const std::string &data_dir_) : data_dir{data_dir_} {
    // Open file for loading
    const std::string data_path = data_dir + "/data.csv";
    int nb_rows = 0;
    FILE *fp = file_open(data_path, "r", &nb_rows);
...
struct EurocTarget {
  ...
  EurocTarget(const std::string &target_file) : file_path{target_file} {
