...
kitti_camera_t *kitti_camera_load(const char *data_dir) {
  ...
  data->timestamps = malloc(sizeof(int64_t) * num_rows);
  ...
  for (int i = 0; i < num_rows; ++i) {
    ...
    int64_t ts = parse_dateline(line);
...
static int64_t *load_timestamps(const char *file_path) {
  ...
  int64_t *timestamps = malloc(sizeof(int64_t) * num_rows);
  for (int i = 0; i < num_rows; ++i) {
    char line[1024] = {0};
    if (fgets(line, sizeof(line), fp) == NULL) {
      KITTI_FATAL("Failed to parse line %d in [%s]!\n", i, file_path);
