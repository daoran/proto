...
void setup_pinhole_radtan4_test(Vec2i &cam_res, VecX &params) {
...
void setup_pinhole_equi4_test(Vec2i &cam_res, VecX &params) {
  cam_res = Vec2i{640, 480};
...
static Vec2 opencv_undistort_point(const VecX &cam_params,
                                     const Vec2 &kp) {
...
TEST(CameraModel, pinhole_radtan4_undistort) {
  ...
  for (int i = 0; i < 100; i++) {
    ...
    const Vec2 kp_xyz = pinhole_radtan4_undistort(params, kp_dist);
