...
int test_p_c2_f_jacobian_wrt_theta1() {
  for (int i = 0; i < 1000; i++) {
    ...
    const vec3_t p_c2_f_1 = (gimbal_model.T_ds() * p_c1_f.homogeneous()).head(3);
    // -- Perturb first gimbal joint angle and tranform again
    gimbal_model.Lambda1 += 1e-4;
    const vec3_t p_c2_f_2 = (gimbal_model.T_ds() * p_c1_f.homogeneous()).head(3);
    ...
    const vec3_t p_c2_f_jacobian = p_c2_f_jacobian_wrt_theta1(
      p_c1_f,
      tau_s,
      tau_d,
      Lambda1,
...
int test_p_c2_f_jacobian_wrt_theta2() {
  for (int i = 0; i < 1000; i++) {
    ...
    // Calculate jacobian using function generated using Matlab
    const vec3_t p_c2_f_jacobian = p_c2_f_jacobian_wrt_theta2(
      p_c1_f,
      tau_s,
      tau_d,
      Lambda1,
