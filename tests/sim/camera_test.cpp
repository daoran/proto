...
int test_VirtualCamera_observedFeatures() {
  ...
  vec3_t rpy{deg2rad(0.0), deg2rad(0.0), deg2rad(0.0)};
  ...
  // Test change in roll
  features.row(0) = vec3_t{10.0, 1.0, 0.0};
  rpy = vec3_t{deg2rad(10.0), deg2rad(0.0), deg2rad(0.0)};
  ...
  features.row(0) = vec3_t{10.0, 0.0, 0.0};
  rpy = vec3_t{deg2rad(0.0), deg2rad(10.0), deg2rad(0.0)};
  t = vec3_t{0.0, 0.0, 0.0};
  feature_ids.clear();
  observed = cam_model.observedFeatures(features, rpy, t, feature_ids);
  ...
  // Test change in yaw
  features.row(0) = vec3_t{10.0, 0.0, 0.0};
  rpy = vec3_t{deg2rad(0.0), deg2rad(0.0), deg2rad(10.0)};
  t = vec3_t{0.0, 0.0, 0.0};
  feature_ids.clear();
  observed = cam_model.observedFeatures(features, rpy, t, feature_ids);
  ...
  features.row(0) = vec3_t{10.0, 0.0, 0.0};
  rpy = vec3_t{deg2rad(0.0), deg2rad(0.0), deg2rad(0.0)};
  t = vec3_t{1.0, 0.0, 0.0};
  feature_ids.clear();
  observed = cam_model.observedFeatures(features, rpy, t, feature_ids);
...
int test_VirtualStereoCamera_observedFeatures_sandbox() {
  ...
  const vec3_t rpy_G{deg2rad(0.0), deg2rad(0.0), deg2rad(0.0)};
  const vec3_t t_G{1.0, 0.0, 0.0};
  const vec3_t rpy_C0{-rpy_G(1), -rpy_G(2), rpy_G(0)};
  const mat3_t R_C0G = euler123ToRot(rpy_C0);
  const vec3_t t_C0 = rotx(-M_PI / 2.0) * rotz(-M_PI / 2.0) * t_G;

  // Do the same for the second camera
  const vec3_t rpy_C1{deg2rad(10), deg2rad(0.0), deg2rad(0.0)};
  const mat3_t R_C1C0 = euler123ToRot(rpy_C1);
  const vec3_t t_C0C1{-0.5, 0.0, 0.0};
  const mat4_t T_cam1_cam0 = transformation_matrix(R_C1C0, t_C0C1);
  const mat4_t T_cam0_cam1 = T_cam1_cam0.inverse();

  const mat3_t R_C1G = T_cam0_cam1.block(0, 0, 3, 3) * R_C0G;
  const vec3_t t_C1 = (T_cam0_cam1 * t_C0.homogeneous()).head<3>();
  ...
  vec3_t p{0.0, 0.0, 10.0}; // Point in camera frame
  vec3_t p0 = camera_model.project(homogeneous(p), R_C0G, t_C0);
  vec3_t p1 = camera_model.project(homogeneous(p), R_C1G, t_C1);
