...
#include "ethz_apriltag/Edge.h"
#include "ethz_apriltag/FloatImage.h"
#include "ethz_apriltag/GLine2D.h"
#include "ethz_apriltag/GLineSegment2D.h"
#include "ethz_apriltag/Gaussian.h"
#include "ethz_apriltag/GrayModel.h"
#include "ethz_apriltag/Gridder.h"
#include "ethz_apriltag/Homography33.h"
#include "ethz_apriltag/MathUtil.h"
#include "ethz_apriltag/Quad.h"
#include "ethz_apriltag/Segment.h"
#include "ethz_apriltag/TagFamily.h"
#include "ethz_apriltag/UnionFindSimple.h"
#include "ethz_apriltag/XYWeight.h"
...
std::vector<TagDetection> TagDetector::extractTags(const cv::Mat &image) {
  ...
  ethz_apriltag::FloatImage fimOrig(width, height);
  int i = 0;
  ...
  for (unsigned int qi = 0; qi < quads.size(); qi++) {
    ...
    if (!bad) {
      ...
      for (int i = 0; i < 4; i++) {
        float const dist =
            ethz_apriltag::MathUtil::distance2D(bottomLeft, quad.quadPoints[i]);
        if (dist < bestDist) {
          bestDist = dist;
          bestRot = i;
        }
