...
BUILD_SUITESPARSE_CMD := \
	if [ -f $(PREFIX)/lib/libsuitesparseconfig.so ]; then return 0; fi \
	  && cd "$(SRC_PATH)" && git clone https://github.com/DrTimothyAldenDavis/SuiteSparse \
...
BUILD_EIGEN_CMD := \
	if [ -f $(PREFIX)/include/eigen3/Eigen/Core ]; then return 0; fi \
	  && cd "$(SRC_PATH)" && git clone https://gitlab.com/libeigen/eigen.git \
		&& cd "$(SRC_PATH)/eigen" \
...
BUILD_CERES_CMD := \
	if [ -f $(PREFIX)/lib/libceres.a ]; then return 0; fi \
	  && cd "$(SRC_PATH)" && git clone https://github.com/ceres-solver/ceres-solver \
...
BUILD_OPENCV_CMD := \
	if [ -f $(PREFIX)/lib/libopencv_core.so ]; then return 0; fi \
	  && cd "$(SRC_PATH)" && git clone https://github.com/opencv/opencv \
		&& cd "$(SRC_PATH)/opencv" \
		&& mkdir -p build \
		&& cd build || return \
		&& cmake \
...
BUILD_APRILTAG_CMD := \
	if [ -f $(PREFIX)/lib/libapriltag.so ]; then return 0; fi \
	  && cd "$(SRC_PATH)" && git clone https://github.com/AprilRobotics/apriltag \
		&& cd "$(SRC_PATH)" && git clone https://github.com/AprilRobotics/apriltag-imgs \
...
BUILD_GLAD_CMD := \
	if [ -f $(PREFIX)/lib/libglad.a ]; then return 0; fi \
		...
		&& rm glad.o \
		&& mv $(SRC_PATH)/glad/libglad.a "$(PREFIX)/lib/libglad.a";

BUILD_GLFW_CMD := \
	if [ -f $(PREFIX)/lib/libglfw3.a ]; then return 0; fi \
		&& cd "$(SRC_PATH)" && git clone https://github.com/glfw/glfw \
		&& cd "$(SRC_PATH)/glfw" \
		&& mkdir -p build \
		&& cd build || return \
		&& cmake \
...
BUILD_ASSIMP_CMD := \
	if [ -f $(PREFIX)/lib/libassimp.so ]; then return 0; fi \
	  && cd "$(SRC_PATH)" && git clone https://github.com/assimp/assimp \
		&& cd "$(SRC_PATH)/assimp" \
		&& mkdir -p build \
		&& cd build || return \
		&& cmake \
