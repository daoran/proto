...
int test_calib_mono() {
  ...
  // const double fx = pinhole_focal(cam_res[0], 100.0);
  // const double fy = pinhole_focal(cam_res[1], 100.0);
  ...
  // const vec4_t dist_params{0.01, 0.01, 0.001, 0.001};
  ...
  LOG_INFO("Adding landmarks to graph!");
  std::vector<id_t> landmark_ids;
  {
    aprilgrid_t grid;
    aprilgrid_set_properties(grid,
                             target.tag_rows,
    ...
    for (const auto &pt : object_points) {
      const auto landmark_id = graph_add_landmark(graph, pt);
      landmark_ids.push_back(landmark_id);
      graph_add_landmark_factor(graph, landmark_id, 1e-8 * I(3));
    }
  }
  ...
  for (const auto &frame : frames_init) {
    ...
    for (const auto &tag_id : grid.ids) {
      ...
      for (int i = 0; i < 4; i++) {
        const auto landmark_id = landmark_ids[(tag_id * 4) + i];
  ...
  for (const auto &kv : graph.factors) {
    ...
    residuals.push_back(factor->residuals(0));
    residuals.push_back(factor->residuals(1));
  }
  printf("RMSE reprojection error: %f\n", rmse(residuals));

  for (const auto &landmark_id : landmark_ids) {
    print_vector("landmark", graph.params[landmark_id]->param);
  }
