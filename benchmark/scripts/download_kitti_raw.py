...
def download_kitti_raw(download_path):
  ...
  for seq in sequences:
    url_path = ""
    if seq[-4:] != ".zip":
      url_path = f"{seq}/{seq}_sync.zip"
    else:
      url_path = f"{seq}"

    dst_path = f"{download_path}/{url_path}"
    os.makedirs(os.path.dirname(dst_path), exist_ok=True)
    download(f"{kitti_url}/{url_path}", dst_path)
    extract_zip(dst_path, os.path.dirname(dst_path))

if __name__ == "__main__":
  download_path = "/data/kitti_raw"
  download_kitti_raw(download_path)
