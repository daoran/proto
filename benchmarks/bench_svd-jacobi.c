  ...
  for (new_row = 0; new_row < n; new_row++) {
    ...
    for (new_column = 0; new_column < n; new_column++) {
      ...
      for (old_x = 0; old_x < n; old_x++) {
        new_matrix[new_row * n + new_column] +=
            m1[new_row * n + old_x] * m2[old_x * n + new_column];
      }
    }
  }
}
...
void generate_composite_matrix(
    double **matrix, int p, int q, int n, double angle, int side) {
...
void order_a(double *a, int n, double *u, double *v) {
  int d_i,
      sort_index; // d_i is diagonal element index, sort_index is index of sort
  ...
  for (row = 0; row < n; row++) {
    for (col = 0; col < n; col++) {
      if (row == col)
        ...
        if (fabs(a[row * n + col]) >
            epsilon) // not yet converged, error > epsilon
          return 1;
      }
    }
  }
